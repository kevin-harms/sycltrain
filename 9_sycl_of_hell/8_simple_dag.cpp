...
class memcopy_kernel{
 public:
   ...
   void operator()(sycl::item<1> idx) { accessorW[idx.get_id()] = accessorR[idx.get_id()] + 1; }
 private:
   TAccessorW accessorW;
   TAccessorR accessorR;
};

void f_copy(sycl::handler& cgh, int global_range, sycl::buffer<sycl::cl_int, 1> bufferW, 
    ...
    cgh.parallel_for(sycl::range<1>(global_range), memcopy_kernel(accessorW, accessorR));
}
