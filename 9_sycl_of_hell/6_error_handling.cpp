...
int main(int argc, char** argv) {
  ...
  sycl::async_handler ah = [](sycl::exception_list elist){ for( auto e : elist) { std::rethrow_exception(e); } };
  ...
  try {
    ...
    myQueue.submit([&](sycl::handler& cgh) {
      ...
      cgh.parallel_for<class hello_world>(sycl::nd_range<1>{sycl::range<1>(global_range), 
          ...
          cout<< "Hello world: World rank/size: " <<  world_rank << " / " << work_size  << sycl::endl;
      }); // End of the kernel function
    }); // End of the queue commands 

    myQueue.wait_and_throw();
  }
  catch (sycl::exception &e) { std::cout << "Async Exception: " << e.what() << std::endl; }
